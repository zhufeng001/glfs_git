                     ...
                     int (*optcbk)(char *word, void *param))
{
        ...
        for (vme = glusterd_volopt_map; vme->key; vme++) {
        		// store all key(w after '.') in param
                w = vme->key;

                for (i = 0; i < lvl; i++) {
                	// in w found every patt
                        if (patt[i]) {
                        		// w come from vme's key
                                w = strtail (w, patt[i]);
                                GF_ASSERT (!w || *w);
                                if (!w || *w != '.')
                                        goto next;
                ...
                // create lots of subnode in para by w
                // stored in param
                ret = optcbk (w, param);
                if (dot)
                        GF_FREE (w);
...
volopt_trie_section (int lvl, char **patt, char *word, char **hint, int hints)
{
        ...
        // store all vmep-> key in trie by volopt_trie_cbk
...
{
		// init vme and get param->rv by param->handler
...
volgen_dict_get (dict_t *dict, char *key, char **value)
{
        ...
        vme.key = key;
        // where is glusterd_volopt_map value come from?
...
{
		// get option complete by key?
        struct volopt_map_entry *vme = NULL;
        // key is part of vme->key
        *completion = NULL;
        for (vme = glusterd_volopt_map; vme->key; vme++) {
...
{
		// assign to key
		// or key is total of vme->key or part or vme->option
        int ret = 0;
...
_gd_get_vmep (char *key) {
        ...
        struct volopt_map_entry *vmep = NULL;
        int ret = 0;
        // if ket not completion ,then return.
        // key should contain '.'
...
{
		// xlator option
		// judge vmep's flag (found by key)
        struct volopt_map_entry *vmep = NULL;
...
{
		// get opt type by key? from vol_opt_list->given_opt
        struct volopt_map_entry *vmep = NULL;
        void                    *dl_handle = NULL;
        ...
        if (vmep) {
                ...
                if (_get_xlator_opt_key_from_vme (vmep, &xlopt_key))
                        goto out;
                // opt from vol_opt_list->given_opt
