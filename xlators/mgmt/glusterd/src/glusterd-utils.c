...
{
		// return val iob share with outmsg. iob got the xdr
        ...
        // iov provide the alloc page and share with outmsg
        // iobuf share with iovec
...
{
		// submit req (convert to reply ) and arg (rsp and convert to proghdr) and payload
        struct iobuf           *iob        = NULL;
        int                     ret        = -1;
        struct iovec            rsp        = {0,};
        char                    new_iobref = 0;
        ...
        // convert arg to rsp (share with iob)
...
{
		// create brickinfo from str brick
        int32_t                 ret = -1;
        glusterd_brickinfo_t    *new_brickinfo = NULL;
        char                    *hostname = NULL;
        char                    *path = NULL;
...
glusterd_volume_brickinfo_get (uuid_t uuid, char *hostname, char *path,
                               ...
                               glusterd_brickinfo_t **brickinfo)
{
		// volinfo to traversal vol->bricks
		// get brick info by hostname(convert to peer_uuid and brick->uuid) and path (brickinfo->path)
        glusterd_brickinfo_t    *brickiter = NULL;
        uuid_t                  peer_uuid = {0};
        int32_t                 ret = -1;

        // cp uuid to peer_uuid
        if (uuid) {
        ...
        ret = -1;
        // compare brickiter->uuid and peer_uuid and path and brickiter->path
        // get brickinfo
...
glusterd_volume_brickinfo_get_by_brick (char *brick,
                                        ...
                                        glusterd_brickinfo_t **brickinfo)
{
		// use tmp_brickinfo to get real brickinfo
        int32_t                 ret = -1;
        glusterd_brickinfo_t    *tmp_brickinfo = NULL;

        GF_ASSERT (brick);
        GF_ASSERT (volinfo);
        // get brickinfo from brick
...
{
		// found volumeinfo by volume_id in priv-.volumes
        int32_t                 ret = -1;
        xlator_t                *this = NULL;
                                 ...
                                 gf_boolean_t wait)
{
	...
	runner_add_arg (&runner, "--brick-port");
        ...
        // reste brickinfo's port and rdma_port
        brickinfo->port = port;
...
{
		// found peerinfo by uuid from priv->peers
        int                     ret = -1;
        glusterd_conf_t         *priv = NULL;
                                  ...
                                  glusterd_peerinfo_t  **peerinfo)
{
        ...
        *peerinfo = NULL;
        ...
        GF_ASSERT (priv);
        // find entry from priv->peers by hoststr.if found return
        list_for_each_entry (entry, &priv->peers, uuid_list) {
        ...
        // hoststr to addr .addr to p(in_addr(to host)) (p to hname)
        // use hname and host to find in priv->peers
...
glusterd_sm_tr_log_transition_add (glusterd_sm_tr_log_t *log,
                                   ...
                                   int event)
{
		// log state and event to log->transitions
        glusterd_sm_transition_t *transitions = NULL;
        int                      ret = -1;
        int                      next = 0;
        xlator_t                 *this = NULL;
...
_update_volume_op_versions (dict_t *this, char *key, data_t *value, void *data)
{
        ...
        // reset ctx's op_version and client_op_version
