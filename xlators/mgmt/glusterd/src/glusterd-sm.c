...
{
		// state ?? of whate ? new and old?
		// log state and event_type
		// reset peerinfo -> state
        GF_ASSERT (state);
        GF_ASSERT (peerinfo);
...
{
		// add event to gd_friend_sm_queue
        GF_ASSERT (event);
...
{
		// op by check event_type ? or event
        if (!event)
                return;
...
{
		// check peerinfo-> state and old_sate and event_type
...
glusterd_friend_sm ()
{
        ...
        while (!list_empty (&gd_friend_sm_queue)) {
                list_for_each_entry_safe (event, tmp, &gd_friend_sm_queue, list) {
                		// get event from gd_friend_sm_queue
                	    // get info from event
