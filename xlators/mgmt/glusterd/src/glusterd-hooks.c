...
{
		// set key and vlaue to runner until false
        int     i               = 0;
        int     count           = 0;
        int     ret             = -1;
        char    query[1024]     = {0,};
                          ...
                          glusterd_commit_hook_type_t type)
{
        ...
        for (lineno = 0; lineno < line_count; lineno++) {
                ...
                runinit (&runner);
                // runner containd of hooks_path entry->d_name,volname ,--fist ,--last
                // key(replaced by other) and value(replaced by other )(come from op_ctx)
...
glusterd_hooks_post_stub_enqueue (char *scriptdir, glusterd_op_t op,
                                  dict_t *op_ctx)
{
		// create stub use args and add stub to conf->hooks_priv
        int                     ret     = -1;
