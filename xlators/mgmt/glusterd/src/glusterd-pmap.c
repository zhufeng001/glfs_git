...
nextword (char *str)
{
		// find next word skip uncomplete word and space
                      ...
                      gf_pmap_port_type_t type)
{
        ...
        for (p = pmap->base_port; p <= pmap->last_alloc; p++) {
                ...
                for (brck = pmap->ports[p].brickname;;) {
                		// brick starts with brickname
                        nbrck = strtail (brck, brickname);
                        // find brick and p by brickname
                        if (nbrck && (!*nbrck || isspace (*nbrck)))
                                return p;
                        // brck contain not only on brickname ?
                        brck = nextword (brck);
                        if (!*brck)
                                break;
                }
...
pmap_registry_search_by_xprt (xlator_t *this, void *xprt,
                              gf_pmap_port_type_t type)
{
		// find port by xprt and type
		// port is ports struct index
        struct pmap_registry *pmap = NULL;
        int                   p    = 0;
        int                   port = 0;
...
{
		// find a free port and set port's type and return
        struct pmap_registry *pmap = NULL;
        int                   p = 0;
        int                   port = 0;
...
pmap_registry_remove (xlator_t *this, int port, const char *brickname,
                      gf_pmap_port_type_t type, void *xprt)
{
		// remove port find by brickname xprt and type
		// set ports[p] is null
        struct pmap_registry *pmap = NULL;
        int                   p = 0;
        glusterd_conf_t      *priv = NULL;
