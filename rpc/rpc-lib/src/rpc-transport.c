...
{
	// malloc trans and set trans
	// trans_name is volume name ?
	...
	// get transport-type from options
	...
	if (ret < 0) {
        ...
        */
		// get bind-insecure from options and set trans
    ...
    // get ops init fini reconfigure from  dlsym handle set to trans
	...
	*VOID(&(trans->fini)) = dlsym (handle, "fini");
    ...
    // malloc vol_opt
    // get vol_opt given_opt from handle options
    // insert vol_opt to this->volume_options
...
{
		// set trans notify and mydata of trans
        int32_t ret = -1;
