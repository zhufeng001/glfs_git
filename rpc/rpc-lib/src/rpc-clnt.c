...
rpc_clnt_reconnect (void *trans_ptr)
{
        ...
        conn  = trans->mydata;
        clnt = conn->rpc_clnt;
        // check conn->reconnect and connected
        pthread_mutex_lock (&conn->lock);
        {
                if (conn->reconnect)
                ...
                if (conn->connected == 0) {
                        ...
                        // where conn config come from ?
                        ret = rpc_transport_connect (trans,
                                                     conn->config.remote_port);
                        // reconnect is event struct
                        conn->reconnect =
                                gf_timer_call_after (clnt->ctx, tv,
                                                     rpc_clnt_reconnect, // event.cbk
                                                     trans); // event.data
                        							// event->cbk(data)
                } else {
                        gf_log (trans->name, GF_LOG_TRACE,
...
{
		// delete event conn->timer and conn->ping_timer
	    // destroy conn->saved_frames and create a new one
...
{
		// when be called ?
		// called in rpc_transport_notify ??
		// check event
		// trans->notifyfn
        rpc_clnt_connection_t  *conn        = NULL;
        struct rpc_clnt        *clnt        = NULL;
        ...
        switch (event) {
        // if disconnect then reconnect ; and notify to rpc with brickid
                          ...
                          dict_t *options, char *name)
{
        int                    ret  = -1;
        rpc_clnt_connection_t *conn = NULL;
        // set conn and conn->trans conn->saved_frame
        conn = &clnt->conn;
        ...
        conn->rpc_clnt = clnt;
        // set conn->trans to rpc_clnt_notify and conn
              ...
              uint32_t reqpool_size)
{
        ...
        // malloc rpc,set rpc->ctx reqpool ,saved_frame_pool
        // rpc->conn auto malloc with rpc
        ...
        // set rpc->conn  and rpc->conn->trans
...
{
		// get conn->lock and return rpc->disabled
        rpc_clnt_connection_t *conn = NULL;
        char                   disabled = 0;
...
rpc_clnt_transport_unix_options_build (dict_t **options, char *filepath,
                                       int frame_timeout)
{
		// set transport dict param and assign to options
        dict_t                  *dict = NULL;
        char                    *fpath = NULL;
        int                     ret = -1;
