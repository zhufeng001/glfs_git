...
{
		// create a sunnode by id and insert into node
		// found subnode by id
...
{
		// create lots of subnode?
		// every node has 255 subnode . not share with other node
		// can store all words
...
{
		// walk all node and subnode. execute fn
        trienode_t *trav = NULL;
        int              i = 0;
        int              cret = 0;
        int              ret = 0;
        // eowonly is 0
        // the topest node eow is 1
        if (!eowonly || node->eow)
                ret = fn (node, data);

        if (ret)
                goto out;
        // scan all node from top to bottom
        for (i = 0; i < 255; i++) {
                trav = node->subnodes[i];
                if (!trav)
                        continue;
...
{
		// every node->id represent a char
		// parent rep the prev char
        if (!node->parent)
                return;
...
{
		// get word from node,from the subnode.
        char *buf = NULL;
